...
- The game context includes "Hand by suit" with EXACT card counts per suit. ALWAYS use those exact numbers.
- Do NOT recount or estimate. If context says "spades (3): J, 8, 4" — they have exactly 3 spades. Not 4, not 2. Three.
- If you reference the player's hand, reference what's actually in the context. Do not hallucinate cards.
`.trim();
// ═════════════════════════════════════════════════════════════════════════════
...
function buildGameContext(context) {
  ...
  if (context.phase) ctx += `Game phase: ${context.phase}\n`;
  if (context.hand) ctx += `Player's hand (full list): ${context.hand}\n`;
  if (context.handBySuit) ctx += `Hand by suit (USE THESE EXACT COUNTS — do not recount): ${context.handBySuit}\n`;
  if (context.bids) ctx += `Bids: ${context.bids}\n`;
  if (context.tricksWon !== undefined) ctx += `Tricks won so far — Us: ${context.tricksWon}, Them: ${context.opponentTricksWon}\n`;
  if (context.tricksNeeded !== undefined) ctx += `Tricks still needed to make bid: ${context.tricksNeeded}\n`;
  if (context.trickStatus) ctx += `TRICK STATUS (critical — read before advising on what to play): ${context.trickStatus}\n`;
  if (context.currentTrick) ctx += `Current trick on the table: ${context.currentTrick}\n`;
  if (context.bags !== undefined) ctx += `Current bags: ${context.bags}\n`;
  if (context.scores) ctx += `Score — Us: ${context.scores.us}, Them: ${context.scores.them}\n`;
  if (context.nilStatus) ctx += `Nil status: ${context.nilStatus}\n`;
  if (context.partnerRead) ctx += `Partner read: ${context.partnerRead}\n`;
  if (context.bidRecommendation) ctx += `Your bid analysis: ${context.bidRecommendation}\n`;
  if (context.playRecommendation) ctx += `Your play analysis: ${context.playRecommendation}\n`;
  if (context.triggerDescription) ctx += `What just happened: ${context.triggerDescription}\n`;
  return ctx;
}
// ═════════════════════════════════════════════════════════════════════════════
...
export function buildTeachingPrompt(topic, gameContext) {
  ...
  return [
    ...
    topic,
    '',
    'Follow the teaching focus instructions above exactly. If the focus says to walk the player through something first, DO THAT before asking any questions. If the focus says to ask a question, ask a question. The focus overrides any default instinct to open with a question.',
